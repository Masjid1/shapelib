...
def _rasterize_matplotlib(geom, pixratio, xrange=None, yrange=None, imageout=None):
    ...
    if imageout:
        ...
        remove = True
    x0, y0, x1, y1 = _geomplotrange(geom, xrange, yrange)
...
def _rasterize_rasterio(geom, pixratio, xrange=None, yrange=None, imageout=None):
    ...
    with rasterio.drivers():
        array = features.rasterize(geoms, out_shape=(cols, rows), transform=transform)
    ...
    array_uint8 /= 255
    assert array_uint8.shape == (cols, rows)
...
def rasterize(geom, pixratio, xrange=None, yrange=None, imageout=None):
    ...
    backends = {
        ...
        "matplotlib": _rasterize_matplotlib
    }
    backends_priorities = [
        "rasterio", 
        "matplotlib"
    ]
    for backend in backends_priorities:
        func = backends[backend]
        out = func(geom, pixratio, xrange, yrange, imageout=imageout)
        if out:
            print("using backend: {backend}".format(**locals()))
            return out
